  ...
  const startRecording = useCallback(
    async (wsReference = null) => {
      ...
      try {
        ...
        mediaRecorderRef.current = new MediaRecorder(stream, AUDIO_CONFIG.MEDIA_RECORDER);

        mediaRecorderRef.current.ondataavailable = (event) => {
          if (event.data.size > 0 && wsReference.readyState === WebSocket.OPEN) {
        ...
        setMicStatus("denied");
        alert("Please allow microphone access to record audio. Error: " + err.message);
      }
    },
    [recordingStatus]
  );
